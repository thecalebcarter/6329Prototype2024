...
public class UpperShooter extends SubsystemBase implements IVelocityControlledSubsystem {
	...
	public CustomTalonFX UpperShooterFalcon = new CustomTalonFX(12);

	public UpperShooter() {
		TalonFXConfigurator uShooterConfigurator = this.UpperShooterFalcon.getConfigurator();
		uShooterConfigurator.apply(new TalonFXConfiguration());
		this.UpperShooterFalcon.clearStickyFaults();

		uShooterConfigurator.apply(new FeedbackConfigs()
			.withFeedbackSensorSource(FeedbackSensorSourceValue.RotorSensor));
		//this.UpperShooterFalcon.configSelectedFeedbackSensor(FeedbackDevice.IntegratedSensor);

		uShooterConfigurator.apply(new SoftwareLimitSwitchConfigs()
		...
		this.UpperShooterFalcon.setInverted(false);
	...
	public void velocityControl() {
		...
		this.UpperShooterFalcon.setControl(new VelocityVoltage(targetVelocity));
	}
	...
	public void resetShooterEncoder() {
        try {
			UpperShooterFalcon.getConfigurator().setPosition(0, .03);
        }
        catch (Exception e) {
	...
	public double getCurrentVelocity() {
		double currentVelocity = this.UpperShooterFalcon.getVelocity().getValueAsDouble();
